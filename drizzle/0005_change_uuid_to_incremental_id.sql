...
DROP TABLE IF EXISTS `transfer`;--> statement-breakpoint
DROP TABLE IF EXISTS `transaction`;--> statement-breakpoint
DROP TABLE IF EXISTS `category`;--> statement-breakpoint
DROP TABLE IF EXISTS `wallet`;--> statement-breakpoint
PRAGMA foreign_keys=ON;--> statement-breakpoint

-- Create wallet table with auto-increment ID
CREATE TABLE `wallet` (
...
CREATE INDEX `idx_wallet_active` ON `wallet` (`user_id`,`is_active`);--> statement-breakpoint

-- Create category table with auto-increment ID
...
-- Create transaction table with auto-increment ID
...
-- Create transfer table with auto-increment ID
...
CREATE INDEX `idx_transfer_date` ON `transfer` (`user_id`,`transfer_date`);
